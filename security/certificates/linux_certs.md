...
1. Download root-ca-template.conf
    * Note that if the GitHub repo is private, one needs to check the URL via browser, so that token is created.
    ~~~    
    wget https://raw.githubusercontent.com/iisti/it-admin-tricks/master/security_certs/root_ca.conf
    ~~~
1. Copy and edit the **root-ca-template.conf** and save it as **root-ca.conf**. 
   * ??? ocsp_url for Root Ca is not really needed as CRL file is super small.
...
* Create OCSP CSR
    ...
    openssl req -new \
        -newkey rsa:2048 \
        -subj "/C=GB/O=Example/CN=OCSP Sub Responder" \
...
* Save the configuration with name **server01_openssl.conf** or something similar.
* Generate server CSR
    ~~~
    server=server01
    openssl req \
...
* Create chain file
~~~
cat server01.crt sub-ca.crt root-ca.crt > server01_chain.crt
~~~ 

## Verify the certificates with OpenSSL
* Verify Sub CA certificate
    ~~~
    openssl verify -verbose -x509_strict -CAfile root-ca.crt -CApath ./ sub-ca.crt server01.crt

    test-sub-ca.crt: OK
    ~~~
* Verify server certificate
    ~~~
    openssl verify -verbose -x509_strict -CAfile test-root-ca.crt -CApath ./ -untrusted test-sub-ca.crt server01.crt

    server01.crt: OK
    ~~~
* Verifying running server
    ~~~
    openssl s_client -CAfile server01_chain.crt -servername server01.domain.com -connect server01.domain.com:443
...
* By default CRL is not checked and revoked certificate will return **0 (ok)** which is odd. The command below will check the CRL with local CRL file.
  * Notice that ***-crl_check*** does not download any CRL files and seems that ***-crl_download*** did not work correctly either.
  ...
  ~~~
  openssl s_client -CAfile server01_chain.crt -servername server01.domain.com -connect server01.domain.com:443 -crl_check -CRL test-sub-ca.crl
  ...
  * One could add the CRL file also to the chain for testing purposes.
  ~~~
  cat server01_chain.crt sub-ca.crl > server01_chain02.crt
  
  openssl s_client -CAfile server01_chain02.crt -servername server01.domain.com -connect server01.domain.com:443 -crl_check
